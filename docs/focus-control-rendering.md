...
> ✅ **상태: 완료(FINISHED) — 능동 작업용 아님(설계근거 박제).** 계층1(UI/분류) + 계층2(라이브
> 미터 피드) **모두 구현·커밋·라이브검증 완료**(커밋 `1074b5e`, 2026-06-25). 본문은 그 설계 기록이며
> 포트→위젯 매핑·모니터 분류는 그대로 유효한 레퍼런스다. 진행 내역은 §10.
> 관련: [`qt-roadmap.md`](qt-roadmap.md), [`ui-design-rules.md`](ui-design-rules.md).
> 작성: 2026-06-25 (mod-web-ui 동작 6에이전트 분석 + 플러그인 라이브러리 서베이 + 계층1·2 구현).
...
**계층2 완료 ✅(커밋 `1074b5e`, 2026-06-25, 라이브검증 OK):**
- 라이브 모니터 피드 = `monitorfeed.py`(의존성無 raw WS)가 mod-ui 웹소켓 수동구독 → `output_set`
  → 미터(~30Hz throttle, `data_ready` ack, Qt 스레드 마샬). A안(웹소켓 구독) 채택.
- 미터 dB 매핑: modmeter 출력 = 선형진폭 0..1 → `20·log10`(-60dB 바닥). 선형 norm이면 바가
  ~1%만 차 안 보이던 것 해결.
- 라이브 검증: HighGainRig 신호 → 바 가시적 움직임. **배포·커밋 완료(서비스 반영됨).**
- 후속(미완, 본 트랙 밖): enum/trigger 위젯 **실조작** 검증, 모니터 tier-2 하드코딩 레지스트리는 빈 채 둠.
