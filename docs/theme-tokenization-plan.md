...
## 0.6 진행 현황 (2026-07-11, 브랜치 `feat/theme-tokenization`)

- ✅ **Phase 0+1 (정본+배선)** `62ff9d0` — `theme/tokens.json`·`theme.py`·`theme_qml.py`·`qt_main` 배선. 시각변화 0.
- ✅ **Phase 2 (색 스윕, 7파일)** `3b8cc10`~`51242fb` — 전 7파일 실제 코드 hex 리터럴 0.
  정본 색 토큰 72개. 오프디바이스 검증: 값 비트동일(editor_bridge는 16버킷·3포트 원본 바이트동일),
  QQmlComponent 컴파일 클린, 토큰 무결성 전부 해결.
  - **렌더되는 시각변화는 딱 2곳(공격적 collapse, 둘 다 sub-perceptual)**: 클립미터>0.92
    `#e6724a`→`#e8694a`, 뱅크 좌/우 패널 `#10141d`→`#0e1118`. 나머지 전부 비트동일.
  - ✅ Pi 색 검증 통과(무변화 확인), master 머지.
- ✅ **Phase 3+4 (폰트 역할토큰 + 스케일 합리화)** `e7e9361` — 27종 px + 굵기를 역할 토큰으로. 완만한 10단
  scale(11~200, display 42·displayLg 56 분리로 큰 타이틀 급축소 방지) 위 semantic 역할. **6화면 그룹**으로
  나눠 각 Pi 육안검증(오버뷰·포커스·보드/스냅·뱅크/SAVE AS·MENU허브/튜너/탭·에디터/피커) 통과. 앱 전 QML
  raw pixelSize 0·uiFont 0, 글리프는 typeSize(family-less). button 역할은 22↔18 왕복 튜닝으로 22 확정
  (역할 한 줄 튜닝 실증). 카탈로그 pins 배지는 반응형 폭으로 오버플로 해결(`4749a07`).
- **→ 테마 토큰화(색+폰트) 전체 완료.** 남은 ④ 잔여 = 문자열 i18n(별건, 미착수).
