...
"정정의 정정". 최종: `meter = Meter()`, 콜백 스레드 `SCHED_FIFO 90`, **한가한 리그에서 xrun은
0과 구별되지 않는다**(처음 "22/10분"으로 적었으나 그건 기동 트랜지언트 + 개발 부하였다 — X (9)).
**실기 헤더**: 무음 IN −59, 드라이브 보드 OUT −14(게인이 잔뜩 걸려 험노이즈만으로). 프로세스
분리는 **불필요**로 판명 — 로드맵 ④에서 내렸다.
...
> (⚠️ 이 표의 "미터 후 22"는 아래에서 다시 무너진다 — 기동 트랜지언트였다. 그리고 "미터 전 ~19"가
> 22와 거의 같다는 것 자체가 뒤의 (9)로 가는 실마리다: 둘 다 실은 내 개발 부하였다.)
>
...
>
> **잔여는 없다.** 미터의 정상상태 비용은 **한가한 리그에서 0과 구별되지 않는다.** 관측된 xrun은
> 전부 **개발 부하**(테스트 스위트 · 몇 시간치 저널 스캔 · 프로브 자신)에 붙어 나고, 유휴 구간은
> 깨끗하다 — 미터 OFF로 **47분 연속 0**, 미터 ON으로 **11분 연속 0이 두 번**.
>
> 이 문단은 **두 번 틀렸다가 여기 왔다.** ① 처음엔 "잔여 22/10분 = `draw`의 꼬리"라고 적었다 —
> 프로브의 0.03/s와 실측 0.037/s가 맞는 걸 보고 **일치를 인과로 읽었다.** 정작 그 10분의 대부분은
> 패널이 꺼져 `draw`가 돌지도 않았다. ② 그래서 패널 상태로 갈라 "2/3는 draw, 바닥 0.019/s는 미상"으로
> 고쳤다. 그것도 틀렸다 — **22는 기동 1분 뒤부터 잰 트랜지언트**였고(정착 후 2.7/10분), 그 사건들의
> 시각이 전부 **내가 그 박스에서 뭔가 돌리던 시각**이었다. 미상이 아니라 **우리였다.**
>
> GIL은 범인이 아니라 잔당이고 주소는 `poll`이 아니라 `draw`다 — **다만 그 잔당이 실제로 xrun을
> 내는지는 미확인이다.** `gil_probe`가 잰 `draw`의 12.5ms GIL 스파이크는 실측이지만, 그게 한가한
> 리그에서 xrun으로 이어지는 걸 본 적은 없다.
>
> **방법론 (8) — 숫자가 맞는 것은 인과가 아니다.** 위 ①. 드러낸 건 [사용자]의 "dim 전 조작 중에만
> 갱신하면 되지 않나" 제안이었다. 제안 자체는 성립하지 않지만(콜백은 우리 폴링과 무관하게 jackd가
> 부르고, 가드는 이미 거기 있다) **가드가 이미 있다는 사실이 곧 "그럼 꺼진 동안 난 건 뭐냐"**였다.
> (3)·(7)과 한 가족 — 틀린 부분, 틀린 소거, 틀린 일치. 셋 다 측정 자체는 정확했다.
>
> **방법론 (9) — 관측자가 부하다.** 오늘 두 번. **알아챈 쪽**: 프로브가 서비스와 I2C를 다퉈 유령
> 제스처가 `combo()`(스냅샷 저장)를 발화시켜 페달보드를 덮어썼다. **못 알아챈 쪽**: 테스트 스위트·저널
> 스캔·프로브가 만든 CPU·I/O가 곧 우리가 "리그의 베이스라인"이라 부르던 xrun이었다. [사용자]가
> **"그냥 개발 때문에 오디오랑 ganglion 말고 다른 프로세스가 도는 순간이란 거잖아"**로 닫았다.
> → **부팅 이후 누적 5760회(0.09/s)를 "이 리그의 DSP"라고 적은 것도 같은 의심을 받아야 한다.**
> 그 박스에서 하루 종일 개발하고 있었다. 리그를 재려면 **리그만 돌고 있어야 한다.**
>
