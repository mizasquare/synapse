...
> [`decisions.md`](decisions.md)(구현 결정 로그 A~S)에, 주제별 상세는
...
>
> **정렬 원칙 — 우선순위 축:** ① 버그 → ② 기능(미구현 본체) → ③ 뷰 폴리시 →
> ④ 온메탈 잔여 → ⑤ 아키텍처·성능 → ⑥ 백로그. (번인 방어 축은 신설된 날 닫혔다 — 위 ✅.)
...
둘 다 finalize 전에 닫아야 한다. 사용자에게 **거짓말을 하는** 종류라 성격이 나쁘다.

- [ ] **패스스루 소실 `[치명]`** — [`../geco_routing.py`](../geco_routing.py):68의 `if fx:`가 IN→OUT
      연결을 감싸고 있다. 트렁크 이펙트가 **하나도 없으면 IN이 OUT에 안 붙어 기타 소리가 죽는다.**
      재현: 이펙트 1개짜리 보드에서 그 자리에 메트로놈을 `place()` → 무음. 빈 체인도 같다.
      ③의 유틸 노드 작업에 딸려 있다. (소)
- [ ] **콤보 스냅샷 저장이 저장을 안 함** — [`../app.py`](../app.py):473-478 `combo()`가 `st.dirty`를
      지우고 `"SNAPSHOT SAVED"` 토스트만 띄운다. **백엔드를 호출하지 않는다.** dirty 점까지 지워서
      사용자에겐 저장된 것처럼 보인다. → ②의 effect 채널(persist emit)과 같은 항목.
      → [`decisions.md`](decisions.md) N "남은 것"(2). (소)
...
- [ ] **effect 채널 (toast/persist emit)** — ②의 콤보 저장 버그와 같은 뿌리. (소)
...
- [`decisions.md`](decisions.md) — **구현 결정 로그(A~S).** 왜 그렇게 정했나 + 라이브 검증 기록.
