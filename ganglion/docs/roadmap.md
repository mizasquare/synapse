...
> 두 줄로 688 → 22. C 샴 · 프로세스 분리 · Zynthian 패턴 · `modmeter`가 **전부 불필요**해졌고,
...
- [x] **레벨미터 배선 (결정 H)** `[해결]`(2026-07-17) — synapse `levelmeter.py`(자체 `jack.Client`,
      ...
      판명돼 ④에서 내렸다). 실기: 무음 IN −59 / 드라이브 보드 OUT −14, xrun 22/10분(리그 베이스라인
      ~19와 같은 자리, 남의 클라이언트 0). 숫자는 live amp가 아니라 **5초 윈도 peak**(synapse `pack()`과
...
- [x] ~~**RT 경로에서 파이썬 빼기 — 미터를 다시 붙이는 유일한 길**~~ **[폐기 2026-07-17 — 문제가
      ...
      두 줄로 688 → 22(남의 클라이언트 11 각각 → 0). 전말·측정치는 [`decisions.md`](decisions.md) X의
      "정정의 정정".
      ...
      - **남은 진짜 항목은 아래 두 개**(`draw` 꼬리, 인코더 폴 34ms)이고, 둘 다 규모가 40배 작다.
- [ ] **`draw`의 꼬리 — 잔여 22/10분의 2/3** — `tools/gil_probe` 실측: `draw`가 p50 6.5ms인데
      **max 80ms**, GIL 지각 max 12.5ms로 60초에 2번 튄다(= 0.03/s). 실측 잔여를 패널 상태로 가르면
      켜짐 **0.065/s** 대 꺼짐 **0.019/s**이고, 그 차이 0.046/s가 프로브의 숫자와 같은 자리다.
      아래 "안 바뀐 상태면 render 건너뛰기"가 그대로 해법 후보. 먼저 잴 것: 80ms 스파이크가 I2C
      쓰기인지 PIL인지(`oled_bench`가 자리에 있다). 급하진 않다 — 패널이 켜져 있는 건 서비스 수명의
      **5.7%**(X 측정)뿐이고, 그 5.7%는 사용자가 미터를 **보고 있는** 시간이라 끌 수도 없다. (중)
- [ ] **바닥 0.019/s — 미터도 `draw`도 아닌 것** ⚠️ **미상.** 패널이 꺼져 `_draw()`도 `observe()`도
      안 도는 6분 동안에도 7건이 났다. 미터가 없던 시절 리그 자신의 베이스라인(~19/10분 = 0.03/s,
      mod-monitor·effect_5/6/8/9에 **분산**)과 같은 자리라, 그 배경 스톨이 이제 그래프에서 **유일하게
      파이썬인** 클라이언트로 **귀속만 옮겨온** 것으로 보인다 — 다만 안 쟀고, 표본도 7건이라 얇다
      (포아송 ±2.6). 즉 이건 미터가 **더한** 게 아니라 리그가 **원래 갖고 있던** 것일 수 있다.
      먼저 볼 것: 이 스톨이 주기적인지(타임스탬프 간격), 미터를 떼면 다시 mod-monitor·effect_*로
      흩어지는지, 그리고 `state = Running`(콜백이 **시작은 했는데** 못 끝냄)이 무엇을 뜻하는지.
      **[사용자] 실전 영향은 없다 — 그냥 기분이 나쁘다.** (소, 조사)
