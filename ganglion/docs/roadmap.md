...
> 마지막 갱신: **2026-07-12 (문서 신설 — 첫 집계. + ③ 노브 스크롤·패치 2×1 완료 반영.)**
>
...
- [ ] **marquee 스크롤** — 128px 폭에서 **긴 이름은 지금 전부 `_fit`으로 잘려나간다.** 패치 2×1이
      값 줄은 살렸지만(→ P) 나머지 라벨은 그대로다. design.md §2가 "한 페이지 밴드 애니메이션은 저렴"으로
      이미 허가한 영역(초기 '마퀴=리스크' 판단은 코스트 모델로 반증됨) — 남은 건 구현. (중)

      **적용 대상 (실측 잘림률, `_fit` 호출부 전수):**

      | 지점 | `app.py` | 티어·폭 | 표본 | **잘림** | 비고 |
      |---|---|---|---|---|---|
      | **보드명** (GLANCE) | `:944` | 24px / 112px | 실 보드 7 | **100%** | 24px는 5~6자만 들어감. 최악 |
      | **스냅명** (GLANCE) | `:950` | 24px / 112px | — | **≈100%** | 위와 동일 티어. `word-요일` 스킴이라 항상 김 |
      | **노드명** (체인 헤더) | `:901` | 16px / 92px | 화이트리스트 40 | **75%** | `NAM Loader`→`NAM LOA`. 열린결정 #8과 한 건 |
      | **패치 값** (2×1) | `:918` | Micro5 / 111px | AIDA 323 | **39%** | NAM·Cab·Reverb는 0% (2×1로 해소됨) |
      | **플러그인명** (피커 fx스트립) | `:1117` | 8px / 반칸 | 화이트리스트 40 | **42%** | 고를 때 구분이 안 되는 게 문제 |
      | **노브명** (반칸) | `:916` | 8px / 52px | 라이브 포트 | **다수** | 반칸=7자. 오프라인 표본이 없어 미측정 |
      | **enum 값** (반칸) | `:918` | 8px / 52px | scale_points | **다수** | 위와 같은 셀 |
      | 카테고리 키 (피커) | `:1131` | 8px / 56px | 8 | 0% | 불필요 |
      | confirm 대상명 | `:1015` | 12px / 104px | 7 | 0% | 불필요 |
      | 생성 이름 (NAMING) | `:1000` | 12px / 122px | — | 0% | 불필요 (12px는 의도된 선택) |

      → **포커스된 한 줄만** 흐르게 하는 게 규율(§2: 매 프레임 애니메이션은 한 페이지 밴드에 격리).
      정적 화면은 지금처럼 trunc 유지. 상위 3개(보드·스냅·노드명)만 해도 체감의 대부분이다.
...
- [`decisions.md`](decisions.md) — **구현 결정 로그(A~O).** 왜 그렇게 정했나 + 라이브 검증 기록.
