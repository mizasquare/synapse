...
def notify_synapsin(message="", socket_path="/tmp/synapsin.sock"):
    """
    Sends a notification to the DIY project via a Unix socket.
    ...
    try:
        ...
        client_socket.close()
        logging.info(f"Notification sent: {str(message)}")
...
class EffectAdd(JsonRequestHandler):
    ...
    def get(self, instance):
        ...
        self.write(data)
        notify_synapsin(message=f"EffectAdd {instance}, {uri}, {x}, {y}")
...
class EffectRemove(JsonRequestHandler):
    ...
    def get(self, instance):
        ...
        self.write(ok)
        notify_synapsin(message=f"EffectRemove {instance}")
...
class EffectParameterSetSYN(JsonRequestHandler):
    @web.asynchronous
    @gen.engine
    ...
    def post(self, port):
        ...
        ok = yield gen.Task(SESSION.syn_parameter_set, port, value)
        self.write(ok)

class EffectParameterGetSYN(JsonRequestHandler):
    @web.asynchronous
    @gen.engine
...
class EffectPatchSetSYN(JsonRequestHandler):
    ...
    def post(self,port):
        ...
        try:
            ...
            ok = SESSION.ws_patch_set(instance, uri, valuetype, valuedata, self)
            self.write(ok)
...
class GeneralCommand(JsonRequestHandler):
    ...
    def post(self, *args):
        ...
        try:
            ...
            self.write({'cmd': cmd, 'value': value})
        except Exception as e:
            self.write({'error': str(e)})
...
class ServerWebSocket(websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if cmd == "data_ready":
            ...
            notify_synapsin(message=f"EffectParameterSet {port}, {value}")
...
class PedalboardLoadBundle(JsonRequestHandler):
    ...
    def post(self):
        ...
        notify_synapsin(message=f"PedalboardLoadBundle {name}")
...
class SnapshotRemove(JsonRequestHandler):
    def get(self):
        ...
        self.write(ok)
        notify_synapsin(message="SnapshotRemove %d" % idx)
...
class SnapshotName(JsonRequestHandler):
    def get(self):
        ...
        notify_synapsin(message="SnapshotName %d" % idx)
...
class SnapshotLoad(JsonRequestHandler):
    ...
    def get(self):
        ...
        self.write(ok)
        notify_synapsin(message=f"SnapshotLoad {idx}")
...
class BankLoad(JsonRequestHandler):
    def get(self):
        ...
        notify_synapsin(message="BankLoad")
...
if __name__ == "__main__":
    ...
    atexit.register(socket_manager.cleanup_socket)
