...
// Pedalboard Editor mock — PyQt6 port of the Claude Design "Pedalboard Editor".
// QUICK (auto-routing) and ADVANCED (manual port-level routing) share one 734x446 canvas.
// A thin dev band on top hosts test triggers; visual tokens live here, data/logic in `editor`.
Window {
    id: win
    visible: true
    width: 800
    ...
    Rectangle {
        ...
        Item {
            ...
            Row {
                ...
                Pill { label: "BOARD"; accent: cBlue; onTap: win.boardsOpen = true }
                Text {
                ...
                Pill { label: "UNDO"; accent: cBorder; dim: !editor.canUndo
                       onTap: if (editor.canUndo) editor.undo() }
                Pill { label: "REDO"; accent: cBorder; dim: !editor.canRedo
                       onTap: if (editor.canRedo) editor.redo() }
                Pill { label: "SHUF"; accent: cGreen; dim: editor.advanced
